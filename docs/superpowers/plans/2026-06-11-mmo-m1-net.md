...
- S→C：`welcome{pid,seed,x,y,z,edits:[[x,y,z,id]],players:[{pid,name,x,y,z,yaw}],online}`、`penter{pid,name,x,y,z,yaw}`、`pmove{pid,x,y,z,yaw,pitch}`、`pexit{pid}`、`edit{x,y,z,id}`、`editReject{x,y,z,id}`（id=该格真实值）、`teleport{x,y,z}`、`online{n}`、`rehello{}`（兜底：无会话且 attachment 恢复失败时要求客户端重新握手）
...
确保 wrangler dev 在跑（`npx wrangler dev --port 8787 --persist-to ../cc-myworld-state`），Run: `node tests/manual/two_clients.js`
Expected: `two_clients PROBE OK`

- [ ] **Step 3: 双窗口浏览器验收（对照 spec 验证标准 1、2、9）**

`npx wrangler dev --port 8787 --persist-to ../cc-myworld-state` 在跑，开两个浏览器窗口（其一用无痕模式，保证 localStorage 凭证不同）访问 `http://127.0.0.1:8787`：
...
- DO 休眠唤醒（本地 workerd 十几秒空闲即触发，属常态而非异常）：会话经 WebSocket attachment + players 表在 boot 时整体恢复（getWebSockets 模式），pid 延续、互见集惰性重建；`rehello` 仅作 attachment 缺失时的兜底。唤醒前最后 ≤30 秒未落盘的位置变化可能丢失
- 本地开发必须 `npx wrangler dev --persist-to <项目外路径>`：assets.directory="." 时 wrangler 的文件监听不理会 .assetsignore，DO/缓存 SQLite 若写在项目内 .wrangler/ 下会自我触发无限热重载
...
- 休眠恢复后 yaw/pitch 归零（players 表不存朝向）：空闲玩家在他人视角朝北站立，其下一次移动即自愈；不为此加表列
