...
| `index.html` | 修改 | 遮罩四模式（连接中/起名/开始/file 提示）、在线人数、script 标签 |
...
### Task 7: index.html 与 ui.js — 遮罩四模式与在线人数
...
(function (root) {
  ...
  // 遮罩内容模式：connecting | name | start | file
  let overlayMode = 'connecting';
  function setOverlayMode(mode) {
    overlayMode = mode;
    const ids = { connecting: 'ovConnecting', name: 'ovName', start: 'ovStart', file: 'ovFile' };
    for (const k in ids) {
...
git commit -m "feat: 遮罩四模式（连接/起名/开始/file提示）与在线人数 UI"
```
...
(function (root) {
  ...
  document.addEventListener('pointerlockchange', () => {
    if (isLocked()) UI.showOverlay(false);
    else if (world && Net.connected()) UI.setOverlayMode('start');
    else if (world) UI.setOverlayMode('connecting'); // 断线触发的解锁：保持「连接中」遮罩
  });
