...
- 箭矢不逐 tick 同步：两端按同一常量独立积分，服务器只广播起点与终点（命中权威在服务器）。自己的箭本地预表现（id 'L#'）与权威箭（'a#'）无对应关系——权威箭命中实体后本地箭会继续飞到撞墙/超时自毁，纯视觉偏差
...
- [x] **Step 3: 浏览器双窗口验收（对照 spec 验证标准 3、4、5、6、10）**
...
## 验收结果（2026-06-13 浏览器验证 + 部署上线）

**Step 2 自动化全绿：** `tests/run_all.js` 11/11、`tests/manual/two_clients.js`、`tests/manual/combat_probe.js` 均 exit 0。

**Step 3 浏览器双窗口验收**（127.0.0.1 + localhost 不同源；按 browser-verify-quirks 用 JS 注入 `player` + `Net.send` 驱动、截图驱动 rAF 帧）：
- ✅ ① 史莱姆营地 / 体素模型 / 「Lv.N 史莱姆」名牌
- ✅ ② 剑：3 连杀、11 次「-3」飘字、受击血条(`hurtMob`)、`pickMob`/挥击；`mobDie`→倒地
- ✅ ③ 反击红闪(`flashRed`)、血条掉血、脱战回血（combat_probe `hpUpdate` 证）
- ✅ ④ 弓：7 发本地箭预表现 + 5 次命中远怪；带下坠补偿瞄准能命中＝弹道含重力
- ✅ ⑤ 死亡黑屏「你死了」+ 约 3.03s 出生点满血复活；HP 持久化由 `world_do.js` 中 hp 与 x/y/z 同条 UPDATE + two_clients 位置持久化共同覆盖
- ✅ ⑦ 双窗口「在线 2」、同种子、互见玩家模型 + 名牌
- ✅ ⑧ 两窗口控制台 0 报错
- ⚠️ ⑥ 脱战回巢：由 `test_mobs` 的 `aiStep`（chase→return→idle+healed）单测 + 服务器 `tickMob` 接线覆盖；浏览器视觉未干净抓取（需锁定具体怪 id 追踪 + 后台标签节流，属环境受限非缺陷）

**手持模型遮挡修复（00e2e44）确认：** 剑/弓手持模型贴视野右下角、不遮挡中心，本地与线上均确认。

**Step 5 部署上线：** `npx wrangler deploy` → https://cc-myworld.xudahy.workers.dev （Version `8080eeef-63c2-460d-bed5-55a315b0438e`）。线上冒烟：M2 模块全加载、连生产 DO、出生点渲染 18 怪、HUD 完整、控制台 0 报错、HTTP 200。

**结论：M2「怪物与战斗」里程碑完成并验收通过，验收期间未发现新功能缺陷。**
